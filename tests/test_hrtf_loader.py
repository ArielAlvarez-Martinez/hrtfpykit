...
def test_parse_directions_rejects_non_spherical_type():
    with pytest.raises(ValueError, match="Only 'spherical' is supported"):
        hrtf_loader._parse_directions(np.array([[0.0, 0.0]]), "cartesian")


def test_hrir_to_hrtf_magnitude_shape_and_freq_bins():
    hrir = np.ones((2, 2, 8), dtype=float)
    mag, freqs = hrtf_loader._hrir_to_hrtf_magnitude(hrir, sampling_rate=48000, fft_length=8)
...
def test_load_multiple_hrtfs_from_folder_with_mocked_loader(monkeypatch):
    ...
    monkeypatch.setattr(hrtf_loader.os, "listdir", fake_listdir)
    monkeypatch.setattr(hrtf_loader, "load_hrtf", fake_load_hrtf)

    out = hrtf_loader.load_from_folder("dummy")
