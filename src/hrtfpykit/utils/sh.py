...
def sht_error(
    ...
    The function compares two magnitude arrays element by element. It
    ...
    finite arrays with matching shape. By default, errors are computed on
    linear magnitudes. With ``magnitude="db"``, absolute magnitudes are
    converted to decibels with a small positive floor before calculating the
    same error metrics, so ``rms_error`` is a global LSD-style RMS dB error for
    the provided arrays.
    ...
    magnitude : {``linear``, ``db``}, default=``linear``
        Domain used for the error calculation. ``linear`` compares the input
        arrays directly. ``db`` compares the decibel values of the absolute
        input magnitudes.
    reference : float | {``max``}, default=1.0
        Reference magnitude used only when magnitude=``db``. Passing ``max``
        uses the maximum absolute magnitude across both input arrays so the
        original and reconstructed values share the same dB reference.
    ...
    tuple[float, float, float, float]
        ...
        - absolute_error: L2 norm of original - reconstructed in the selected
          magnitude domain. With magnitude=``db``, this is the L2 norm of the
          dB-domain reconstruction error.
        - relative_error: absolute error divided by the reference L2 norm.
          .With magnitude=``db``, the reference norm is computed from the original
          dB values.
        - rms_error: root-mean-square point-wise error. With magnitude=``db``,
          this is the global LSD-style RMS error in dB.
        - max_absolute_error: maximum absolute point-wise error. With
          magnitude=``db``, this is the largest point-wise absolute dB error.

    Raises
    ------
    ValueError
        If the input arrays have different shapes, are empty, or contain
        non-finite values, if magnitude is unsupported, if reference is invalid
        for dB conversion, or if dB conversion produces non-finite values.
    ...
    if magnitude_key not in {"linear", "db"}:
    ...
    if magnitude_key == "db":
        ...
        if isinstance(reference, str) and str(reference).strip().lower() == "max":
            resolved_reference = float(
                np.max(
                    np.concatenate(
                        [
                            np.abs(original_values).reshape(-1),
                            np.abs(reconstructed_values).reshape(-1),
                        ]
                    )
                )
            )
        ...
        original_values = np.asarray(
            magnitude_to_db(
                np.maximum(np.abs(original_values), db_floor),
        ...
        reconstructed_values = np.asarray(
            magnitude_to_db(
                np.maximum(np.abs(reconstructed_values), db_floor),
                reference=resolved_reference,
            ),
            dtype=float,
        )
