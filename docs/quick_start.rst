...
Use the :doc:`sofa API</sofa/index>` when you want to handle the file structure directly:
...
Use the :doc:`hrtf API </hrtf/index>` when you want to work with SOFA files that follow the
...
Use the :doc:`datasets API </datasets/index>` to build map style dataset
pipelines from public HRTF datasets. Dataset objects are configured with
:doc:`specs </datasets/specs>`, which declare which acoustic values, cue
