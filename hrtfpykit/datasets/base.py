...
def build_value_signature(value: object) -> object:
    ...
    if isinstance(value, dict):
        return tuple(
            sorted(
                (str(key).strip().lower(), build_value_signature(item))
...
def validate_aligned_asset_spec(
    ...
    if asset_align_by not in supported_align_by:
        raise ValueError(
            f"{asset_name} align_by must be one of {supported_align_by}"
        )
...
def apply_hrtf_spec_transform(
    ...
    if not is_hrtf_object(transformed_hrtf):
        raise ValueError(
...
class BaseDataset:
    ...
    def __init__(
        ...
        if len(hrtf_specs) > 1:
            ...
            for spec in hrtf_specs:
                resolved_variant = (
                    ...
                    else str(self.config.hrtf.default_variant).strip().lower()
                )
            ...
            if len({build_value_signature(spec.positions) for spec in hrtf_specs}) > 1:
                raise ValueError(
                    "All HRTFSpec objects must use the same positions when hrtf is used in both inputs and target"
                )
            if len({build_value_signature(spec.plane) for spec in hrtf_specs}) > 1:
                raise ValueError(
                    "All HRTFSpec objects must use the same plane when hrtf is used in both inputs and target"
                )
            if len({build_value_signature(spec.ears) for spec in hrtf_specs}) > 1:
                raise ValueError(
        ...
        if len(anthropometry_specs) > 1:
            ...
            for spec in anthropometry_specs:
                ...
                if not path.is_absolute():
                    path = self.root / path
                anthropometry_path_signatures.add(build_value_signature(path))
        ...
        primary_hrtf_spec = select_primary_spec(input_hrtf_specs, hrtf_specs)
        primary_mesh_spec = select_primary_spec(input_mesh_specs, mesh_specs)
        primary_anthropometry_spec = select_primary_spec(
            input_anthropometry_specs,
            anthropometry_specs,
            prefer_path=True,
        )
        primary_image_spec = select_primary_spec(
            input_image_specs,
            image_specs,
            prefer_path=True,
        )
        primary_video_spec = select_primary_spec(
            input_video_specs,
            video_specs,
            prefer_path=True,
        )
        ...
        self._hrtf_variant = None
        if len(hrtf_specs) > 0:
            if self.config.hrtf is not None:
                self._hrtf_variant = str(self.config.hrtf.default_variant).strip().lower()
            for spec in hrtf_specs:
                if spec.variant is not None:
                    self._hrtf_variant = str(spec.variant).strip().lower()
                    break
        ...
        if primary_hrtf_spec is not None:
            ...
            if self._hrtf_variant is None:
                raise ValueError("hrtf_variant could not be resolved")
            if self._hrtf_variant not in self.config.hrtf.variants:
                raise ValueError(
                    f"Unsupported hrtf_variant {self._hrtf_variant!r}. "
        ...
        if primary_hrtf_spec is not None:
            ...
            self._selected_position_indices = resolve_positions_selection(
    ...
    def get_spec_value(
        ...
        if isinstance(spec, HRTFSpec):
            ...
            use_hrtf_transform = (
                spec.transform is not None and is_explicit_hrtf_transform(spec.transform)
            )
            if (
                spec.transform is not None
                and not use_hrtf_transform
                and is_raw_hrtf_transform_method(spec.transform)
            ):
                raise ValueError(
            ...
            if use_hrtf_transform:
                ...
                if transformed_hrtf is None:
                    transformed_hrtf = apply_hrtf_spec_transform(hrtf, spec.transform)
                    if self._cache_hrtf:
                        self._transformed_hrtf_cache[transform_cache_key] = transformed_hrtf
            value = select_hrtf_value(
