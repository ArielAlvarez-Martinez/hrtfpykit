...
def load_hrtf(
    ...
    resolved_subject_id = type(dataset)._resolve_dataset_subject_id(
        subject_id,
        subject_ids,
    )
    if resolved_subject_id not in dataset._hrtf_paths:
        raise KeyError(
            f"Subject {subject_id!r} resolved to {resolved_subject_id!r} but does not have an available HRTF file"
        )
    path = dataset._hrtf_paths[resolved_subject_id]
    if not path.exists():
        warnings.warn(
            f"{dataset._name}: subject {resolved_subject_id} HRTF path is missing: {path}",
            stacklevel=2,
        )
        raise FileNotFoundError(
            f"HRTF path is missing for subject {resolved_subject_id}: {path}"
        )
    cache_key = ("hrtf", resolved_subject_id)
    ...
    if loaded_hrtf is None:
        try:
            ...
            warnings.warn(
                f"{dataset._name}: subject {resolved_subject_id} HRTF file could not be loaded: {path} ({exc})",
                stacklevel=2,
            )
            raise
    ...
    if resolved_extension == ".csv":
        ...
        with resolved_path.open("r", encoding="utf-8-sig", newline="") as file:
            ...
            if reader.fieldnames is None or len(reader.fieldnames) == 0:
                raise ValueError(f"Anthropometry file {resolved_path} does not contain headers")
            ...
            for row in reader:
                ...
                if raw_subject_id is None or str(raw_subject_id).strip() == "":
                    continue
                try:
                    subject_id = type(dataset)._resolve_dataset_subject_id(
                        raw_subject_id,
                        dataset_subject_ids,
                    )
                except ValueError:
